...
import { generateReminders, type Reminder } from "@/lib/reminders/generate";
import {
    ...
    React.useEffect(() => {
        ...
        const run = async () => {
            ...
            const overdue = results
                ...
                .map((r: any) => ({ label: r.vaccineName, sub: r.reason, date: r.dueDate }));
            ...
            const recent = (user.vaccines ?? [])
                ...
                .map((v: any) => ({ label: v.name, sub: "Last dose", date: v.lastDoseDate as string }))
                .filter((v: any) => {
                    const d = new Date((v.date ?? "") + "T00:00:00").getTime();
                    const diffDays = Math.floor((today.getTime() - d) / (1000 * 60 * 60 * 24));
            ...
            const raw = generateReminders(
                results.filter(
                    (r: any) =>
                        (r.status === "dueSoon" || r.status === "overdue" || r.status === "eligibleNow") &&
            ...
            // 5) history
            const allHistory = loadSentReminderLog().filter((r) => r.userId === userId);
            const activeKeys = new Set(out.map((r) => makeKey(r)));
            const historyOnly = allHistory.filter((r) => !activeKeys.has(makeKey(r)));

            setHistoryReminders(historyOnly.slice().sort((a, b) => +new Date(b.createdAt) - +new Date(a.createdAt)));
        };
    ...
    return (
        <main className="p-6 max-w-5xl mx-auto space-y-6">
            <div className="flex items-center justify-between gap-4">
                <div>
                    <h1 className="text-xl font-semibold">Vaccine Eligibility & Reminders</h1>
            ...
            <Card className="rounded-2xl">
                ...
                <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <MiniBucket title="Overdue" icon={<AlertTriangle className="h-4 w-4" />} items={buckets.overdue} />
                    <MiniBucket title="Due soon" icon={<Clock className="h-4 w-4" />} items={buckets.dueSoon} />
                    <MiniBucket title="Eligible" icon={<CheckCircle2 className="h-4 w-4" />} items={buckets.eligible} />
                    <MiniBucket title="Recent" icon={<History className="h-4 w-4" />} items={buckets.recent} />
                </CardContent>
            </Card>
                ...
                <CardContent>
                    <Tabs defaultValue="active">
                        <TabsList className="grid w-full grid-cols-2">
                            ...
                            <TabsTrigger value="history">Past / Sent ({historyReminders.length})</TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="history" className="mt-4">
                            {historyReminders.length === 0 ? (
                                <p className="text-sm text-muted-foreground">No past/sent reminders for this user.</p>
                            ) : (
                                <div className="space-y-2">
        ...
        <div className="border rounded-lg p-3">
            <div className="flex justify-between gap-3">
                <div className="min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground truncate">{r.vaccineName}</p>
                </div>
